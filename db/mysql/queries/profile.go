...
const (
	...
	UpdateUserProfile = `UPDATE profiles SET first_name = COALESCE(?, first_name), last_name = COALESCE(?, last_name), avatar = COALESCE(?, avatar), updated_at = now() WHERE uid = ?`

	DeleteUserProfile = `UPDATE profiles SET deleted_at = now() WHERE uid = ?`
)
